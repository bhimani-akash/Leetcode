...
of past rounds may affect future rounds' scores.
    ...
    public static int calPoints(String[] ops) {
        ...
        for (int i = 0; i < ops.length; i++) {
            ...
            if ("C".equals(operation)) {
                score -= records[recordIndex - 1];
                records[--recordIndex] = 0;
            } else if ("D".equals(operation)) {
                int currentScore = records[recordIndex - 1] * 2;
                score += currentScore;
                records[recordIndex++] = currentScore;
            } else if ("+".equals(operation)) {
                int currentScore = records[recordIndex - 1] + records[recordIndex - 2];
                score += currentScore;
                records[recordIndex++] = currentScore;
            } else {
                score += Integer.parseInt(operation);
                records[recordIndex++] = Integer.parseInt(operation);
            }
        }
