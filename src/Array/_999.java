...
direction until it chooses to stop, reaches the edge of the board, captures a black pawn, or is blocked by a
white bishop. A rook is considered attacking a pawn if the rook can capture the pawn on the rook's turn.
    ...
    public int numRookCaptures(char[][] board) {
        ...
        for (int i = 0; i < board.length; i++) {
            for (int j = 0; j < board[0].length; j++) {
                if (board[i][j] == 'R') {
                    ans += goUp(board, i, j) ? 1 : 0;
                    ans += goDown(board, i, j) ? 1 : 0;
                    ans += goLeft(board, i, j) ? 1 : 0;
                    ans += goRight(board, i, j) ? 1 : 0;
                    return ans;
                }
            }
        }
